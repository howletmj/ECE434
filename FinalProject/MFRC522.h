...
class RC522 {
	public:
        ...
        uint8_t ATQA_Buffer[2];
                ...
                void SPI_Start(void);
		void closer(void);
	RC522(const char* devices, char bites, uint32_t speeds);
        private:
                void AntennaOn(void);

                uint8_t REQA_or_WUPA_Request(uint8_t Request);
                uint8_t CommunicateWithPICC(uint8_t Command, uint8_t ValidBits, uint8_t InterruptRequestBits, uint8_t SendCount, uint8_t *SendValues,uint8_t ReturnCount, uint8_t *ReturnValues);
